	...
	public function index(...$args)
	{
		...
		if (preg_match('/^[1-9][0-9]*(.html)?$/', $path_array[count($path_array) - 1]) && count($path_array) > 1) {
			...
			if (preg_match('/page[1-9][0-9]*$/', $path_array[count($path_array) - 1])) {
				//带分页的分类
				$page =(int)str_replace("page", "", $path_array[count($path_array) - 1]);
			...
			BaseService::debug()->start("加载分类模板");
			if (is_array($theCategory) && count($theCategory) > 0) {
				...
				if ($thecat) {
					//分类正确
					//获取子类，分类应该包括其子类的所有文章
					$this->data["term_id"] = $thecat["term_id"];
					$this->data["page"] = $page;
					$this->data["url"] = $newPath;
					/*
					...
					*/
					...
					$rootCategory =getTheParentsCategory($thecat["term_id"]); //获取该分类父类，用来做面包屑导航
					$this->data["rootCategory"] = array_reverse($rootCategory, true);  //面包屑
					foreach ($rootCategory as $cat) {
						if ($this->data["title"] == "") {
							$this->data["title"] = langval($cat["terms_name"]);
						} else {

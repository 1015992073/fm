...
if ($conn->connect_error) {
	...
	$sql = "SELECT * FROM `posts` LIMIT 0 , 20";
	$result = $conn->query($sql);

$t2_2 = microtime(true); //获取程序1，结束的时间
echo '</br>查找文章用时:' . (($t2_2- $t1) * 1000) . 'ms' . "<br>";
$ls=getfetch($result);
//var_dump($ls);
$t2_3 = microtime(true); //获取程序1，结束的时间
echo '</br>处理文章用时:' . (($t2_3- $t2_2) * 1000) . 'ms' . "<br>";


$t4_2 = microtime(true); //获取程序1，结束的时间
$sqltol = "SELECT post_id FROM `posts` where post_parent=1 ";
$result = $conn->query($sqltol);
$t4_3 = microtime(true); //获取程序1，结束的时间
echo '</br>总数用时:' . (($t4_3- $t4_2) * 1000) . 'ms' . "<br>";

$showhtml = microtime(true); //
foreach($ls as $post){
?>
<div class="ui-item">

<h3>标题:<a href="<?php echo $post["post_url"]; ?>"><?php echo $post["post_title"]; ?></a></h3>
<p>文章id:<?php echo $post["post_id"]; ?></p>
<p>分类ID:<?php echo $post["post_parent"]; ?></p>

<p>链接:<?php echo  $post["post_url"]; ?></p>
<p>简介:<?php echo $post["post_excerpt"]; ?></p>
</div>
<?php

}

$showhtmlend = microtime(true); //获取程序1，结束的时间
echo '</br>显示列表用时:' . (($showhtmlend- $showhtml) * 1000) . 'ms' . "<br>";

$sql1 = "SELECT * FROM category LIMIT 0 , 1000";
$result1 = $conn->query($sql1);
$t31 = microtime(true); //获取程序1，结束的时间
echo '</br>查询分类用时:' . (($t31- $t2_3) * 1000) . 'ms' . "<br>";
$ls2=getfetch($result1);
echo "分类</br>";
$t32 = microtime(true); //获取程序1，结束的时间
echo '</br>处理分类用时:' . (($t32- $t31) * 1000) . 'ms' . "<br>";
...
function getfetch($result = null)
	...
	function table_exists($tablename = null,$conn)
	{
		if (isset($tablename)) {
...
<footer>
	<div class="environment">
		...
		<p>Page rendered in 10 seconds</p>
...
<?php
$t2_1 = microtime(true); //获取程序1，结束的时间

echo '</br>总用时:' . (($t2_1 - $t1) * 1000) . 'ms' . "<br>";
?>
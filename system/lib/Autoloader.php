...
class Autoloader
{
	...
	{
		//$psr4=array("lib"=>LIBPATH,"app"=>APPPATH,"config"=>APPPATH.'Config'.DS);//自动加载命名空间目录  命名空间=>目录
		$psr4=array("system"=>SYSTEMPATH,"app"=>APPPATH);//自动加载命名空间目录  命名空间=>目录
		$classmap=[];//单独的类文件
		
		if (isset($psr4)) {
			$this->addNamespace($psr4); //将自定义psr4 命名空间加入
		}
